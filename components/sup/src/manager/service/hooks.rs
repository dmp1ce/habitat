...
impl HookTable {
              ...
              U: AsRef<Path>
    {
        if let Some(meta) = std::fs::metadata(templates.as_ref()).ok() {
            if meta.is_dir() {
                self.file_updated = FileUpdatedHook::load(sg_id, &hooks, &templates);
                self.health_check = HealthCheckHook::load(sg_id, &hooks, &templates);
                self.suitability = SuitabilityHook::load(sg_id, &hooks, &templates);
                self.init = InitHook::load(sg_id, &hooks, &templates);
                self.reload = ReloadHook::load(sg_id, &hooks, &templates);
                self.reconfigure = ReconfigureHook::load(sg_id, &hooks, &templates);
                self.run = RunHook::load(sg_id, &hooks, &templates);
                self.smoke_test = SmokeTestHook::load(sg_id, &hooks, &templates);
            }
        }
